...
- **Completed locally**:
  - Fixed tag search to match real tag names and counts instead of asset ids.
  - Added collection result callbacks from `GlobalSearchModal`.
  - Added App-level asset filter targeting so tag and collection results open the Asset Workspace.
  - Added Asset Workspace filter target initialization for tag and collection requests, including user notice and handled callbacks.
  - Avoided `setState` inside target-handling effects by remounting the Asset Workspace on new search filter requests and using the target as initial state.
- **Validation so far**:
...
- **Commit/CI**: `a9b5f7d uiux: clarify global search feedback`; pushed to `main`; GitHub Actions run `28410366920` passed all steps.
...
- **Commit/CI**: `346dfff feat: manage saved global searches`; pushed to `main`; GitHub Actions run `28419346834` passed all steps.
...
- **Final status**: Campaign 031 is complete at 6/6 stages. Global Search now targets exact messages, routes asset filters, explains result/keyboard state, saves reusable queries, tolerates invalid saved data, and lets users remove saved queries.
- **Next**: Start the next campaign from a newly verified workflow need; no known Global Search blocker remains from this campaign.
