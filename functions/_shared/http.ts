...
export async function ensureSchema(db: D1Database) {
  await db.batch([
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_reactions_message ON reactions (message_id)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_reactions_conversation ON reactions (conversation_id, created_at DESC)`),
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_message_edits_message ON message_edits (message_id, created_at)`),
  ]);
}
